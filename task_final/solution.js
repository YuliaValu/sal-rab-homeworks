...
// }
    ...
    function sendRequest(name, phone, address, goods, sum) { 
        ...
        let addressSendForClient = "ул. " + address.street + ", дом " + address.house + ", " + address.entrance + " подъезд, " + address.floor + " этаж, кв " + address.flat;
        let order = {address : addressSendForClient, sum:sum }; 
        let goodsSendForClient = []; 
        let countOfGoods = goods.length; 
        for (let i = 0; i < countOfGoods; i ++) { 
            let x = { title: goods[i].title, count:goods[i].count }; 
            goodsSendForClient.push(x); 
        } 
        let data = {client:client, order:order,goods:goodsSendForClient }; 
